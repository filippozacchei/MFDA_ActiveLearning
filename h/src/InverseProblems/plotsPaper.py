...
def plot_sensitivity_boxplot(samples, true_values, model_path, config_file):
    ...
    for sample in samples:
        ...
        predicted_sensitivities = model.predict(sample_transposed).flatten()
    ...
    for i, true_value in enumerate(true_values):
        ax1.boxplot(sensitivities[i], positions=[true_value], widths=0.02, patch_artist=True, showfliers=False, zorder=1, boxprops=dict(facecolor='lightblue', color=box_colors['boxes']), medianprops=dict(color=box_colors['medians']), whiskerprops=dict(color=box_colors['whiskers']), capprops=dict(color=box_colors['caps']))
        ax1.text(true_value -(-1)**i*0.02, np.mean(sensitivities[i])+0.001, sample_labels[i], horizontalalignment='center', verticalalignment='bottom', fontsize=12)
    # Line of equality
    # Line of equality
...
def plot_sensitivity_histogram(sample, true_value, model, title=None):
...
<<<<<<< Updated upstream
    ...
    plt.axvline(true_value, color='blue', linestyle='dashed', linewidth=3, label = 'True')
    plt.fill_betweenx([0, max(n)], ci_lower, ci_upper, color='gray', alpha=0.35, label='95% CI')
