...
1. Robustness wrt to GP Kernel (if possible, try NN kernel)
1. Overall Efficiency in terms of HF evaluations and total CPU time
1. Robustenss wrt to initial MCMC point
1. Effects of various uncertianty modelling
...
- When a new point is added:
  \[
  K\_{n+1} = \\begin{bmatrix} K_n & k \\ k^T & k\_{new} \\end{bmatrix}
  \]
  Update ( L\_{n+1} ) using Schur complement logic to avoid full recomputation.
