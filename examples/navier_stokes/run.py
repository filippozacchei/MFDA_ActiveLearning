"""Runs the Navier-Stokes (BFS outlet-profile) method comparison
(hf_only/pretrained/adaptive_surrogate_mcmc/adaptive_stm, see `ns_methods`) across
...
Mirrors `examples/toy_problem/run.py` exactly -- same comparison harness, same
per-seed output shape -- with only the problem-specific glue (`ns_methods`,
`ns_plots`) swapped in. See that file's module docstring for anything not repeated
here.

Requirements
------------
Every seed here spends real HF budget on `solver_hf.forward_model`, which requires
the Navier-Stokes/FEniCSx stack (`dolfinx`, `mpi4py`, `petsc4py`, `basix`). This
script has not been run in an environment with that stack installed; it is verified
by `mypy --strict` and by manual cross-checking against the execution-tested MSD side,
which exercises the exact same `gp_active_mcmc.verification` call surface.
"""
...
def run_one_seed(
    ...
    *,
    ...
    posterior/MCMC comparison entirely -- including `hf_only_reference`, by far the
    most expensive part of a seed here (every step a real, uncached FEM solve), which
    `run_training_cost_comparison` never needs -- for fast iteration on just the
    training-cost numbers. Mutually exclusive with `skip_training_cost` (enforced by
    the CLI, not here)."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    problem.add_argument(
        ...
        "(much slower) posterior/MCMC comparison entirely -- including hf_only_reference, the most "
        "expensive part of a seed here -- for fast iteration on training-cost settings. Mutually "
        "exclusive with --skip-training-cost.",
    )
